const mongoose = require("mongoose");
const FollowService= async (Request, UsersModel) => {



    try{


        let followUserID=Request.body['followUserID'];
        let LoginUserID=Request.headers.id;
        const ObjectId = mongoose.Types.ObjectId;
        let QueryObject = {_id: new ObjectId(followUserID)};
        let UserData = await UsersModel.aggregate([{$match: QueryObject}])

        const followers = UserData[0].followers;
        let alreadyFollowed = followers.find((currentValue)=>currentValue.toString() === LoginUserID.toString());

        if(!alreadyFollowed){
            let UpdateData1 = await UsersModel.updateOne(QueryObject, {
                $push: { followers: LoginUserID }
            })

            let UpdateData2 = await UsersModel.updateOne({_id: new ObjectId(LoginUserID)}, {
                $push: { following: followUserID }
            })

            return {message: "success", result:"User Followed"}

        }else{
            return {message: "success", result:"User is already followed by you"}
        }



    }
    catch (error) {
        return {message: "fail", data: error}
    }
}
module.exports=FollowService

